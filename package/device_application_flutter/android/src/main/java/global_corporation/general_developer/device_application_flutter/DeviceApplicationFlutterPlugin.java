...
import global_corporation.general_developer.device_application_flutter.listener.DeviceApplicationFlutterChangedListener;
import global_corporation.general_developer.device_application_flutter.listener.DeviceApplicationFlutterChangedListenerInterface;
import global_corporation.general_developer.device_application_flutter.utils.AppDataConstants;
import global_corporation.general_developer.device_application_flutter.utils.AppDataEventConstants;
import global_corporation.general_developer.device_application_flutter.utils.IntentUtils;
...
import static global_corporation.general_developer.device_application_flutter.utils.Base64Utils.encodeToBase64;
import static global_corporation.general_developer.device_application_flutter.utils.DrawableUtils.getBitmapFromDrawable;

/**
 * DeviceApplicationFlutterPlugin
 */
public class DeviceApplicationFlutterPlugin implements
        FlutterPlugin,
        MethodCallHandler,
        EventChannel.StreamHandler,
        DeviceApplicationFlutterChangedListenerInterface {
    ...
    private DeviceApplicationFlutterChangedListener appsListener;

    public DeviceApplicationFlutterPlugin() {
    ...
    public void onAttachedToEngine(@NonNull FlutterPluginBinding binding) {
        ...
        methodChannel = new MethodChannel(messenger, "global_corporation/general_developer/device_application_flutter");
        methodChannel.setMethodCallHandler(this);

        eventChannel = new EventChannel(messenger,
                "global_corporation/general_developer/device_application_flutter_events");
    ...
    public void onMethodCall(MethodCall call, @NonNull final Result result) {
        switch (call.method) {
            case "getInstalledApps":
                ...
                boolean includeAppIcons = call.hasArgument("include_app_icons")
                        && (Boolean) (call.argument("include_app_icons"));
                boolean onlyAppsWithLaunchIntent = call.hasArgument("only_apps_with_launch_intent")
                        && (Boolean) (call.argument("only_apps_with_launch_intent"));
            ...
            case "getApp":
                if (!call.hasArgument("package_name") || TextUtils.isEmpty(call.argument("package_name").toString())) {
                    ...
                    boolean includeAppIcon = call.hasArgument("include_app_icon")
                            && (Boolean) (call.argument("include_app_icon"));
    ...
    private void fetchInstalledApps(final boolean includeSystemApps, final boolean includeAppIcons,
            final boolean onlyAppsWithLaunchIntent, final InstalledAppsCallback callback) {
        asyncWork.run(new Runnable() {

            @Override
            public void run() {
                List<Map<String, Object>> installedApps = getInstalledApps(includeSystemApps, includeAppIcons,
    ...
    public void onListen(Object arguments, final EventChannel.EventSink events) {
        if (context != null) {
            if (appsListener == null) {
                appsListener = new DeviceApplicationFlutterChangedListener(this);
            }
